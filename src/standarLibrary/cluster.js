...
const requestHandler = (request, response) => {
  ...
  if (request.url === "/error") {
    ...
    response.end(
      `<h1 style="text-align:center;margin-top:40px;">It runs &#128640;</h1>`
    );
...
if (cluster.isPrimary) {
  ...
  cluster.on("listening", (worker, address) => {
    console.log(
      `The worker #${worker.id} is now connected to port #${JSON.stringify(
        address.port
      )}`
    );
  ...
  server.listen(port + cluster.worker.id, (error) => {
    ...
    console.log(
      `Server running 🚀 at http://localhost:${port + cluster.worker.id}/`
    );
  });

  process.on("uncaughtException", (error) => {
    console.error(
      `${new Date().toUTCString()} uncaught exception: ${error.message}`
    );
